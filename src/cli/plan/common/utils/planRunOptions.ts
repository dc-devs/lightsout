 ...
 * Extracted because dedup, draft and grade each spelled the same nine lines out
 * in full — a change to how the config reaches a runner had three places to go.
 */
export const planRunOptions = ({ cwd, driver, name, plansDir, standards, config }: Params): PlanRunOptions => ({
	cwd,
	driver,
	name,
	plansDir,
