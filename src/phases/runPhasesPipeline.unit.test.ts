...
/** A manifest from one of the other pipelines, as `resume` would hand it to the coordinator. */
...
test('runPhasesPipeline: a second fresh start for the same overview is refused with the unfinished run id', async () => {
	const { dir, overviewPath } = setupPhasedRepo({ phases: 2 });
	const config = await loadConfig({ cwd: dir });
	const stopped = await runPhasesPipeline({
		cwd: dir,
		driver: createPhaseDriver({ dir, seen: [], failAt: 1 }),
		config,
		overviewPath,
		skipRefactor: true,
	});
	...
	const error = await getRejectionError({
		promise: runPhasesPipeline({ cwd: dir, driver: createPhaseDriver({ dir, seen: [] }), config, overviewPath, skipRefactor: true }),
	});
...
test.each([
...
])('runPhasesPipeline: resuming $label here is refused and names the door for the $named pipeline', async ({ pipeline, named, door }) => {
	const { dir } = setupPhasedRepo({ phases: 1 });
	...
	const error = await getRejectionError({
		promise: runPhasesPipeline({
			...
			driver: createPhaseDriver({ dir, seen: [] }),
			config: await loadConfig({ cwd: dir }),
			existing: foreignManifest({ pipeline }),
			skipRefactor: true,
		}),
	});
...
test('runPhasesPipeline: a fresh sequence with no overview path is refused', async () => {
	const { dir } = setupPhasedRepo({ phases: 1 });

	const error = await getRejectionError({
		promise: runPhasesPipeline({ cwd: dir, driver: createPhaseDriver({ dir, seen: [] }), config: await loadConfig({ cwd: dir }), skipRefactor: true }),
	});
...
test('runPhasesPipeline: an overview that is not on disk is refused, naming the path it looked for', async () => {
	const { dir } = setupPhasedRepo({ phases: 1 });
	...
	const error = await getRejectionError({
		promise: runPhasesPipeline({
			...
			driver: createPhaseDriver({ dir, seen: [] }),
			config: await loadConfig({ cwd: dir }),
			overviewPath: join('plans', 'demo', 'missing.md'),
			skipRefactor: true,
		}),
	});
...
test('runPhasesPipeline: an overview with no Phases table rows is refused rather than run as an empty sequence', async () => {
...
])('runPhasesPipeline: --start-phase $startPhase ($label) is refused before any state is written', async ({ startPhase }) => {
	const { dir, overviewPath } = setupPhasedRepo({ phases: 2 });
...
test('runPhasesPipeline: a phase file the table names but disk lacks is refused upfront, not at that phase', async () => {
	const { dir, overviewPath } = setupPhasedRepo({ phases: 2 });
...
test('runPhasesPipeline: an absolute overview path is recorded the way this repo stores plan paths', async () => {
	const { dir } = setupPhasedRepo({ phases: 1 });
	...
	const result = await runPhasesPipeline({
		...
		driver: createPhaseDriver({ dir, seen: [] }),
		config: await loadConfig({ cwd: dir }),
		overviewPath: join(dir, 'plans', 'demo', 'overview.md'),
		skipRefactor: true,
	});

	expect(result.ok).toBe(true);
	// stored cwd-relative, so the guard recognises the same overview named either way
	expect(result.manifest.plan).toBe(join('plans', 'demo', 'overview.md'));
});

test('runPhasesPipeline: table rows that name no file are skipped, so a stray row cannot invent a phase', async () => {
	const { dir, overviewPath } = setupPhasedRepo({ phases: 1 });
	const seen: number[] = [];

	writeFileSync(
		join(dir, 'plans', 'demo', 'overview.md'),
		'# Feature — Overview\n\n## Phases\n\n| # | File | Scope |\n|---|------|-------|\n| a stray one-column row |\n| 1 | `phase1.md` | scope |\n| 2 | phase2.md | no code span |\n',
	);
	...
	expect(result.ok).toBe(true);
	expect(result.manifest.steps.map((step) => step.id)).toStrictEqual(['phase1.md']);
	expect(seen).toStrictEqual([1]);
});

test('runPhasesPipeline: a finished sequence for the same overview does not block a fresh one', async () => {
	const { dir, overviewPath } = setupPhasedRepo({ phases: 1 });
	...
	expect(result.ok).toBe(true);
	// only an unfinished sequence is mid-flight; a passed one is history
	expect(result.manifest.runId).not.toBe('finished-sequence');
});

test('runPhasesPipeline: an unfinished sequence for a different overview does not block this one', async () => {
	const { dir, overviewPath } = setupPhasedRepo({ phases: 1 });
	...
	expect(result.ok).toBe(true);
	expect(result.manifest.runId).not.toBe('other-plans-sequence');
});

test('runPhasesPipeline: a run whose manifest no longer parses is skipped rather than treated as unfinished', async () => {
	const { dir, overviewPath } = setupPhasedRepo({ phases: 1 });
...
test('runPhasesPipeline: the refusal names the most recently updated unfinished sequence', async () => {
	const { dir, overviewPath } = setupPhasedRepo({ phases: 1 });
	...
	expect(error.message).toContain('lightsout resume --run newer-sequence');
	expect(error.message).not.toContain('older-sequence');
});
