...
interface Params {
	...
	files: string[];
	/** Per-repo clone floor (config `standardsChecks.clone.settings.minTokens`, default 50). */
	minTokens?: number;
}
 ...
 */
export const checkClones = async ({ cwd, files, minTokens = defaultMinTokens }: Params): Promise<StandardsFinding[]> => {
