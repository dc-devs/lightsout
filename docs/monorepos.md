...
`packageGates` lets lightsout gate only the packages affected by a run. Each command template runs once per affected package, in parallel, with `{package}` replaced by the package’s `package.json` name:
...
**Changes outside the packages directory still get verified.** When a run changes files outside `packagesDir`, lightsout also runs the repository-wide commands configured under `gates`.
