...
describe('path-test-support-in-src check', () => {
	...
	test('reports a shared fixtures folder living under src/, naming the folder rather than its files', async () => {
		...
		expect(findings).toStrictEqual([
			{
				siteKey: 'path-test-support-in-src:src/feature/fixtures',
	...
	test.each([{ folder: 'fixtures' }, { folder: 'mocks' }, { folder: 'testUtils' }, { folder: 'test-utils' }])(
		...
		async ({ folder }) => {
			...
			expect(findings.map(({ siteKey }) => siteKey)).toStrictEqual([`path-test-support-in-src:src/feature/${folder}`]);
		},
	);

	test('leaves a co-located __mocks__ alone, which the same prose sanctions, and helpers, which another rule owns', async () => {
	...
	test('reports each misplaced folder once however many files it holds, in path order', async () => {
		...
		expect(findings.map(({ siteKey }) => siteKey)).toStrictEqual(['path-test-support-in-src:src/a/fixtures', 'path-test-support-in-src:src/b/mocks']);
	});
