...
describe('barrel-under-common check', () => {
	...
	test('reports a barrel sitting in a type folder under common/', async () => {
		...
		expect(findings).toStrictEqual([
			{
				siteKey: 'barrel-under-common:src/billing/common/utils/index.ts',
	...
	test('reports a barrel sitting directly in a common/ folder', async () => {
		...
		expect(findings).toStrictEqual([
			{
				siteKey: 'barrel-under-common:src/billing/common/index.ts',
	...
	test('reports a barrel however deep it sits beneath common/, naming the folder it was found in', async () => {
		...
		expect(findings).toStrictEqual([
			{
				siteKey: 'barrel-under-common:src/billing/common/utils/rates/index.ts',
	...
	test.each([
		...
		expect(findings).toStrictEqual([
			{
				siteKey: `barrel-under-common:src/billing/common/utils/${barrel}`,
	...
	test('reports every offending barrel separately, so each one carries its own site key', async () => {
		...
		expect(findings.map((finding) => finding.siteKey)).toStrictEqual([
			'barrel-under-common:src/billing/common/utils/index.ts',
			'barrel-under-common:src/pay/common/types/index.ts',
		]);
	});
	...
	test('reports a JavaScript-spelled barrel under common/, since the shared name test answers for every dialect', async () => {
		...
		expect(findings.map(({ siteKey }) => siteKey)).toStrictEqual(['barrel-under-common:src/common/utils/index.mjs']);
	});
