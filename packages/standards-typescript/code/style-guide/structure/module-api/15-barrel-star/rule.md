...
A barrel is the module's **public API contract** — it lists exactly what consumers may use; everything it omits is internal. It is a list, not an import path: consumers import each name from the file that declares it, and only files the barrel exports.

1. **Every folder-module has an `index.ts`** — the list of files other modules may import
