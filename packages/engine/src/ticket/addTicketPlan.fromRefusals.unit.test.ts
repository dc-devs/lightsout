...
const ticketBranch = 'lo-157-merge-adopt';
...
const setupFromRefusal = ({ sources = {}, runs = [], lock, config = localOnlyConfig }: SetupParams = {}) => {
	...
	mkdirSync(plansFolderOf({ folder: ticketBranch }), { recursive: true });
	...
	for (const run of runs) {
		const planName = run.planName ?? ticketBranch;
	...
	return {
		cwd,
		plansFolderOf,
		recordPath: join(cwd, '.lightsout', 'tickets', ticketBranch, 'ticket.json'),
		planFolder: join(plansFolderOf({ folder: ticketBranch }), planId),
		params: { cwd, ticketBranch, slug, config, env },
	};
};
...
describe('addTicketPlan', () => {
	test("refuses while a live run's plan lies in the source folder and moves nothing", async () => {
		const { params, plansFolderOf, planFolder, recordPath } = setupFromRefusal({
			sources: { [ticketBranch]: looseFiles },
			runs: [{ runId: 'run-live', status: 'running' }],
			lock: { runId: 'run-live', pid: process.pid },
		});

		const result = await addTicketPlan({ ...params, from: ticketBranch });

		expect(result).toEqual({ error: expect.stringContaining('run-live') });
		expect({
			source: entriesOf({ dir: plansFolderOf({ folder: ticketBranch }) }),
	...
	test('refuses a --from value that is a plan address or is not a bare folder name', async () => {
		const { params, plansFolderOf, planFolder, recordPath } = setupFromRefusal({ sources: { [ticketBranch]: looseFiles } });

		const address = await addTicketPlan({ ...params, from: `${ticketBranch}/${planId}` });
		const empty = await addTicketPlan({ ...params, from: '' });
		const separated = await addTicketPlan({ ...params, from: 'shaped/under-a-slug' });
		const here = await addTicketPlan({ ...params, from: '.' });
		const above = await addTicketPlan({ ...params, from: '..' });
		...
		expect({ address, empty, separated, here, above }).toEqual({
			address: { error: expect.stringContaining(`${ticketBranch}/${planId}`) },
	...
	test('refuses a --from folder with no loose files and names the folder it looked for', async () => {
		...
		const absent = await addTicketPlan({ ...params, from: 'lo-158-never-here' });
		const emptied = await addTicketPlan({ ...params, from: 'lo-158-emptied' });
