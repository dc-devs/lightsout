...
// the worktree, the ticket folder and the plan folders are all real files,
...
/** The ticket folder's name, which is also the branch the linked worktree below stands on. */
...
const syncStateOf = ({ ticketFolder }: { ticketFolder: string }) =>
	JSON.parse(readFileSync(join(ticketFolder, 'ticket-sync.json'), 'utf8')) as { planMarkers: Record<string, string> };
 ...
 * a ticket record naming one plan the ticket has republished since this machine
...
const setupWorktreeKeep = ({ planFolderIn }: { planFolderIn: 'primary' | 'worktree' }) => {
	...
	const planFolder = join(planFolderIn === 'primary' ? primary : worktree, '.lightsout', 'tickets', ticketBranch, 'plans', planId);
	const progress: string[] = [];

	mkdirSync(ticketFolder, { recursive: true });
	writeFileSync(join(ticketFolder, 'ticket.json'), asFileText({ value: recordOf({ plans: [planOf()] }) }));
	...
	mockGetTicketAttachments.mockResolvedValue([{ id: 'att-record', title: 'ticket.json', url: 'https://assets.example/ticket-record' }]);
	...
	return {
		...
		ticketFolder,
		progress,
		params: { cwd: worktree, ticketBranch, config, env, target, onProgress: (message: string) => progress.push(message) },
	};
};

describe('keepLocalTicketRecord', () => {
	test('republishes the divergent plan from the primary checkout when the keep runs in a linked worktree', async () => {
		const { params, primary, worktree, ticketFolder } = setupWorktreeKeep({ planFolderIn: 'primary' });

		const result = await keepLocalTicketRecord(params);
		...
		expect(published).toStrictEqual({ cwd: primary, name: `${ticketBranch}/${planId}`, titlePrefix: planId });
		expect(markersOf({ result })).toStrictEqual({ [planId]: republishedMarker });
		expect(syncStateOf({ ticketFolder }).planMarkers).toStrictEqual({ [planId]: republishedMarker });
