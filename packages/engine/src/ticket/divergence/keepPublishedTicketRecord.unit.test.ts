...
const setupWorktreeKeep = ({ planFolderIn }: { planFolderIn: 'primary' | 'worktree' | 'nowhere' }) => {
	...
	writeFileSync(join(ticketFolder, 'ticket.json'), asFileText({ value: recordOf({ plans: [planOf({ publishedMarker: 'b'.repeat(64) })] }) }));
	...
	const bodies: Record<string, string> = {
		'ticket.json': asFileText({ value: published }),
...
describe('keepPublishedTicketRecord', () => {
	test("sets the primary checkout's plan folder aside and restores the published copy there when the keep runs in a linked worktree", async () => {
		const { params, worktree, ticketFolder, published } = setupWorktreeKeep({ planFolderIn: 'primary' });
	...
	test('sets no folder aside in the worktree, so a folder only the tree holds is left alone and the published copy lands in the primary', async () => {
		const { params, ticketFolder, worktreeTicketFolder } = setupWorktreeKeep({ planFolderIn: 'worktree' });
	...
	test('restores the published copy into the primary checkout when no checkout holds a folder to set aside', async () => {
		const { params, ticketFolder } = setupWorktreeKeep({ planFolderIn: 'nowhere' });
