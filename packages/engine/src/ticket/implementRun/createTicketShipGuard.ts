...
/**
 * Build the ticket record's say over shipping, for whichever caller is about to
 ...
 */
export const createTicketShipGuard = ({ config, env, onProgress }: Params): ShipTicketGuard => ({
	authorize: async ({ cwd, branch }) => {
		// The branch IS the ticket folder's name: every plan address is keyed by its
		...
		if ('error' in pulled) {
			const local = await readTicketRecord({ cwd, ticketBranch: branch });
			...
			// before ticket records existed. A branch that DOES have a record is
		...
		const eligibility = readTicketShipEligibility({ record: pulled.record });
	...
	recordShipped: async ({ cwd, branch, mergeCommit }) => {
		...
		// A branch with no record ships exactly as it did before ticket records
		...
		const updated = await updateSyncedTicketRecord({
			...
			change: (current) => {
				if (current === undefined) {
					return { error: `ticket ${branch} no longer has a record, so the merge ${mergeCommit} could not be recorded on it` };
				}
