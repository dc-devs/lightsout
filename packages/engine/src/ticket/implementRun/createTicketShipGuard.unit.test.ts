 ...
 * and the ticket folder is a path the row can name: whatever record the branch
...
const setupGuard = async ({
	...
	mockGetTicketAttachments.mockResolvedValue(published === undefined ? [] : [{ id: 'att-1', title: 'ticket.json', url: assetUrl }]);
	mockReadTicketAsset.mockResolvedValue(published === undefined ? { error: 'the ticket carries no ticket.json' } : JSON.stringify(published));
	...
	if (record !== undefined) {
		const seeded = await updateLocalTicketRecord({ cwd, ticketBranch, change: () => record });
	...
	return {
		...
		progress,
		branch: ticketBranch,
		guard: createTicketShipGuard({ config, env, onProgress: (message: string) => progress.push(message) }),
	};
};
...
describe('createTicketShipGuard', () => {
	...
	test('authorizes a multiple-plan ticket only while its ship request covers exactly its implemented non-excluded plans', async () => {
		...
		const added = await updateLocalTicketRecord({
			cwd,
			ticketBranch: branch,
			change: (current) =>
				current === undefined
	...
	test('keeps authorizing a ship request after a plan is only retitled', async () => {
		...
		const retitled = await retitleTicketPlan({ cwd, ticketBranch: branch, plan: '002-fix-x', title: 'Fix search ranking', config: localConfig, env: {} });
	...
	test('records the merge commit and the shipped plans on the ticket record', async () => {
	...
	test('reports a shipped mark it could not record as a progress line without throwing', async () => {
		...
		expect(progress).toEqual([expect.stringContaining('ticket.json')]);
	});
