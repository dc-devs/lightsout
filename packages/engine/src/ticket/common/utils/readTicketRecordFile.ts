...
const readRecordText = ({ text, recordPath, ticketBranch }: { text: string; recordPath: string; ticketBranch: string }) => {
	let value: unknown;
	let outcome: { record: TicketRecord } | { error: string } | undefined;
 ...
 */
export const readTicketRecordFile = async ({ recordPath, ticketBranch }: Params): Promise<{ record: TicketRecord | undefined } | { error: string }> => {
