...
/** Moving a folder out from under a run that is still editing it is the one thing adoption cannot do. */
export const findLiveRunRefusal = async ({ primaryCheckout, runs }: Params): Promise<string | undefined> => {
	...
	return live === undefined
		? undefined
		: `run ${live.runId} is still building ${live.plan}, and adopting the folder would move the files out from under it — wait for that run to finish, or stop it, and run this again`;
};
