 ...
 */
const setupFrom = async ({ source = ticketBranch, files = loosePlanFiles, record, runs = [], planFolders = [], sourceSiblings = [] }: SetupParams = {}) => {
	...
	for (const name of planFolders) {
		...
		writeFileSync(join(ticketFolder, 'plans', name, 'plan.md'), `# ${name}\n`);
	}
	...
	return { cwd, ticketFolder, plansFolder: join(ticketFolder, 'plans'), sourceFolder, sourcePlans };
};
...
const addPlan = async ({ cwd, from, title }: { cwd: string; from?: string; title?: string }) => {
	const result = await addTicketPlan({ cwd, ticketBranch, slug, title, from, config: localOnlyConfig, env });
...
describe('addTicketPlan', () => {
	test("moves the named folder's loose files into the new plan and leaves nothing loose beside it", async () => {
		const { cwd, ticketFolder, plansFolder } = await setupFrom();
		const planFolder = join(plansFolder, planId);

		const { change, error } = await addPlan({ cwd, from: ticketBranch });
		...
		expect(recordAt({ ticketFolder })?.plans).toEqual([expect.objectContaining({ id: planId })]);
	});
	...
	test('takes away the emptied source folder and leaves one that still holds something', async () => {
		...
		// takes away only what is provably empty, so this ticket folder stands.
	...
	test("never retires the source when it is the ticket's own folder", async () => {
		...
		const { error } = await addPlan({ cwd, from: ticketBranch });

		expect({ error, ticketFolder: existsSync(ticketFolder), plans: entriesOf({ dir: plansFolder }) }).toStrictEqual({
			error: undefined,
			ticketFolder: true,
			plans: [planId],
		});
	...
	test('puts back every file it had already moved when one of the moves fails and says where the files are', async () => {
		const { cwd, ticketFolder, plansFolder, sourcePlans } = await setupFrom({
			source: sourceBranch,
		...
		expect({
			...
			planned: recordAt({ ticketFolder })?.plans.map((plan) => plan.id),
	...
	test('records plan-adopted for the --from form and plan-added without it', async () => {
		...
		expect({
			from: recordAt({ ticketFolder: fromSource.ticketFolder })?.history.map(({ kind }) => kind),
			plain: recordAt({ ticketFolder: empty.ticketFolder })?.history.map(({ kind }) => kind),
	...
	test('takes --title with --from exactly as it does without it', async () => {
		...
		expect({
			titled: recordAt({ ticketFolder: titled.ticketFolder })?.plans.at(0)?.title,
			untitled: recordAt({ ticketFolder: untitled.ticketFolder })?.plans.at(0)?.title,
