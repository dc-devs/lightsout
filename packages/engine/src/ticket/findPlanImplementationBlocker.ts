...
const findLowerPlanBlocker = ({ record, planId }: { record: TicketRecord; planId: string }) => {
	...
	return blocking === undefined
		? undefined
		: `plan ${blocking.id} comes before ${planId} on ticket ${record.branch} and its implementation has not finished, and a ticket's plans implement in numeric order — finish it with ${describeLowerPlanRemedy({ branch: record.branch, plan: blocking })}`;
};
