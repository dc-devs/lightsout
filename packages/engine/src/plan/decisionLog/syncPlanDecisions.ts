 ...
 * a phased plan keeps one history rather than one per phase. What a plan *is* is
 ...
 * deliverable. Callers already holding the merged record pass it in: the draft
 * flow syncs mid-draft from the record it started with, and a second read of the
 * workspace there would be a second source of truth.
 */
export const syncPlanDecisions = async ({ cwd, name, decisions }: Params): Promise<SyncPlanDecisionsResult> => {
	...
	if (!isSinglePlan && deliverable.overviewPath === undefined) {
		return {
			...
			error: `cannot sync decisions for '${name}': phase files need an overview.md to carry the Decision Log the phases point at`,
		};
	}
	...
	const paths = [...overviewPaths, ...deliverable.files.map((file) => file.path)];
