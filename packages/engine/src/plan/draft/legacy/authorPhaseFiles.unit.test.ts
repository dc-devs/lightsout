...
const setupFanOutFromWorktree = ({ count, driver }: { count: number; driver: Driver }) => {
	...
	const workspaceDir = join(primary, '.lightsout', 'work-orders', 'demo', 'plans');
...
describe('authorPhaseFiles', () => {
	...
	test('a phase claiming a file it never wrote returns failed naming that path', async () => {
		...
		expect(result.error).toContain(join(cwd, '.lightsout', 'work-orders', 'demo', 'plans', 'phase2-step.md'));
		expect(existsSync(join(cwd, '.lightsout', 'work-orders', 'demo', 'plans', 'phase2-step.md'))).toBe(false);
	});
