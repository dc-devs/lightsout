...
export const getCurrentPlanningReviews = ({ snapshot, stage }: Params): PlanningReviewReceipt[] => {
	...
	for (const { receipt, work, baseline } of candidates) {
		...
		const policy = readPlanningExecutionPolicy({ snapshot, stage: work.stage });
		if (
			policy &&
			(invocation.executionPolicyDigest !== policy.reference.sha256 ||
				baseline.executionPolicyDigest !== policy.reference.sha256 ||
				invocation.invocationPolicyDigest !== policy.policy.roles[work.role].digest)
		)
			continue;
