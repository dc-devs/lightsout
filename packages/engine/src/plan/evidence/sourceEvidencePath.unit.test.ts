...
test("sourceEvidencePath: the evidence record sits inside the plan's own workspace folder", async () => {
	...
	expect(forOnePlan).toBe(join(cwd, '.lightsout', 'tickets', 'lo-142-focused-drafting', 'plans', 'source-evidence.json'));
	expect(forAnother).toBe(join(cwd, '.lightsout', 'tickets', 'rate-limit-banner', 'plans', 'source-evidence.json'));
});
...
test("sourceEvidencePath: the record asked for from a linked worktree is the primary checkout's own", async () => {
	...
	expect(path).toBe(join(realpathSync(primary), '.lightsout', 'tickets', 'lo-142-focused-drafting', 'plans', 'source-evidence.json'));
});
