...
describe('resolveShipSettings', () => {
	test('a config with no ship block still resolves, on the defaults the engine names', () => {
		...
		expect(settings).toEqual(expect.objectContaining({ pullRequestBody: '{ticket}', mergeMethod: 'merge', afterImplement: false }));
	...
	test('a configured block wins over every default, compiled pattern included', () => {
		const { config } = setupConfig({
			ship: { 'ticket-pattern': '^(?<ticket>ENG-(?<number>\\d+))', 'pr-body': 'Closes {ticket}', 'merge-method': 'squash', 'after-implement': true },
		});

		const settings = resolveShipSettings({ config });

		expect(settings).toEqual(expect.objectContaining({ pullRequestBody: 'Closes {ticket}', mergeMethod: 'squash', afterImplement: true }));
