...
/** A ticket whose two plans are exactly the set a ship request must name. */
...
/** The same ticket once the request is on its record, bound to both plans. */
...
const setupRequestShip = ({ args, result = { record } }: { args: string[]; result?: ChangeResult }) => {
	...
	const cwd = mkdtempSync(join(tmpdir(), 'lightsout-ticket-request-ship-command-'));
...
describe('ticketRequestShipCommand', () => {
	test('requests with --plans and withdraws with --withdraw', async () => {
		...
		await expect(ticketRequestShipCommand(requesting.context)).rejects.toThrow(/process\.exit/);
		...
		// them against the ticket's plans
		...
		await expect(ticketRequestShipCommand(withdrawing.context)).rejects.toThrow(/process\.exit/);
	...
	test('names the plans a recorded request binds the ticket to, and says a withdrawn one leaves it open', async () => {
		const requested = setupRequestShip({ args: ['--name', 'lo-140-x', '--plans', '1,2'], result: { record: requestedRecord } });

		await expect(ticketRequestShipCommand(requested.context)).rejects.toThrow(/process\.exit/);

		// the stored ids, not the tokens that were typed, are what the ticket is
		// now held to — so those are what the command reads back
		...
		await expect(ticketRequestShipCommand(withdrawn.context)).rejects.toThrow(/process\.exit/);

		// with no request left, the ticket ships on nobody's authority: the line
		// has to say that rather than name a set of plans
		expect(withdrawn.logged.join('\n')).toContain('no ship request');
		expect(withdrawn.exitCodes).toStrictEqual([0]);
	});

	test('refuses a request-ship call that gives both or neither of --plans and --withdraw', async () => {
		const both = setupRequestShip({ args: ['--name', 'lo-140-x', '--plans', '1', '--withdraw'] });

		await expect(ticketRequestShipCommand(both.context)).rejects.toThrow(/process\.exit/);

		// recording a request and withdrawing one are opposite changes to the same
		// field, so the command refuses rather than picking one
		...
		await expect(ticketRequestShipCommand(neither.context)).rejects.toThrow(/process\.exit/);
