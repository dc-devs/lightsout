...
/**
 * `lightsout ticket request-ship` at the terminal.
 *
 ...
 */
export const ticketRequestShipCommand = async ({ flags, cwd }: CommandContext): Promise<void> => {
	...
	if ((plans !== undefined) === withdraw) {
		console.error(
			'`lightsout ticket request-ship` takes exactly one of --plans <id,id> and --withdraw: the first records a request, the second takes one back',
		);
