...
/**
 * `lightsout ticket retitle-plan` at the terminal.
 *
 ...
 */
export const ticketRetitlePlanCommand = async ({ flags, cwd }: CommandContext): Promise<void> => {
	...
	await finishTicketChange({ ticketBranch, outcome, describe: () => [`the plan is now titled '${title}' on ticket ${ticketBranch}`] });
};
