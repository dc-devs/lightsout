...
import { plansDir, planWorkspaceDir } from '#src/plan/index.ts';
 ...
 * folder is there is that one plan. A directory holding plan subfolders is a
 ...
 * must not need several commands and hand arithmetic. A directory holding no
 ...
 *
 * Anything else answers an error naming the value and the directory that was
...
export const resolveReportTargets = async ({ cwd, name }: Params): Promise<{ names: string[]; ticketFolder: boolean } | { error: string }> => {
	...
	if (children === undefined) {
		return { error: `no plan folder named '${name}' under ${await plansDir({ cwd })}` };
	}
