...
import { listEligibleTickets } from '#src/queue/tracker/jira/listEligibleTickets.ts';
...
describe('Jira listEligibleTickets', () => {
	test('queries every route with quoted JQL, requested fields, and token paging', async () => {
		const request = jest
			.fn<(params: unknown) => Promise<unknown>>()
			.mockResolvedValueOnce({ issues: [issue], isLast: false, nextPageToken: 'next' })
			.mockResolvedValueOnce({ issues: [], isLast: true })
			.mockResolvedValueOnce({ issues: [issue], isLast: true });
		mockRunJira.mockImplementation(({ request: call }) => call({ request }));
		const settings = jiraQueueSettingsFixture({ project: 'L"O', eligibleStatuses: ['Ready\\Now', 'Back"log'] });

		const result = await listEligibleTickets({ settings });

		expect(request).toHaveBeenNthCalledWith(1, {
			method: 'POST',
			path: '/rest/api/3/search/jql',
			body: {
				jql: 'project = "L\\"O" AND labels = "route-direct" AND status IN ("Ready\\\\Now", "Back\\"log")',
	...
	test.each([
		...
		const result = await listEligibleTickets({ settings: jiraQueueSettingsFixture() });

		expect(result).toEqual([expect.objectContaining({ route: 'direct', priority }), expect.objectContaining({ route: 'auto-plan', priority })]);
	});

	test('returns without making a request when no status is eligible', async () => {
		mockRunJira.mockClear();

		expect(await listEligibleTickets({ settings: jiraQueueSettingsFixture({ eligibleStatuses: [] }) })).toStrictEqual([]);
	...
	test('fails a nonfinal page without a token', async () => {
		...
		expect(await listEligibleTickets({ settings: jiraQueueSettingsFixture() })).toStrictEqual({
	...
	test('propagates malformed descriptions and required-field absences', async () => {
		...
		expect(await listEligibleTickets({ settings: jiraQueueSettingsFixture() })).toStrictEqual({ error: "Jira issue 'LO-1' has a malformed description" });
	});
	...
	test('uses Jira absence fallbacks for description, labels, priority, links, and status', async () => {
		...
		expect(await listEligibleTickets({ settings: jiraQueueSettingsFixture() })).toEqual([
			expect.objectContaining({ description: '', priority: 0, unfinishedBlockers: [] }),
			expect.objectContaining({ description: '', priority: 0, unfinishedBlockers: [] }),
		]);
	});
});
