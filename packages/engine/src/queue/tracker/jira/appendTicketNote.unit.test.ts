...
import { appendTicketNote } from '#src/queue/tracker/jira/appendTicketNote.ts';
...
describe('Jira appendTicketNote', () => {
	test('reads ADF, inserts under the shared section, and writes ADF to the literal issue path', async () => {
		...
		expect(await appendTicketNote({ settings: jiraQueueSettingsFixture(), ticketId: 'LO/1', heading: '## Decisions', line: '- second' })).toBeUndefined();
	...
	test('initializes a missing description and keeps a repeated answer idempotent', async () => {
		...
		await appendTicketNote({ settings: jiraQueueSettingsFixture(), ticketId: 'LO-1', heading: '## Decisions', line: '- answer' });
		await appendTicketNote({ settings: jiraQueueSettingsFixture(), ticketId: 'LO-1', heading: '## Decisions', line: '- answer' });
	...
	test('rejects malformed nonempty ADF without writing and passes REST failures through', async () => {
		...
		expect(await appendTicketNote({ settings: jiraQueueSettingsFixture(), ticketId: 'LO-1', heading: '## Decisions', line: '- answer' })).toStrictEqual({
