...
import { setTicketStatus } from '#src/queue/tracker/jira/setTicketStatus.ts';
...
describe('Jira setTicketStatus', () => {
	test('reads transitions and posts the first matching destination', async () => {
		...
		expect(await setTicketStatus({ settings: jiraQueueSettingsFixture(), ticketId: 'LO/1', statusName: 'In Progress' })).toBeUndefined();
	...
	test('names a missing destination and does not post', async () => {
		...
		expect(await setTicketStatus({ settings: jiraQueueSettingsFixture(), ticketId: 'LO-1', statusName: 'Done' })).toStrictEqual({
	...
	test('passes a tracker failure through unchanged', async () => {
		...
		expect(await setTicketStatus({ settings: jiraQueueSettingsFixture(), ticketId: 'LO-1', statusName: 'Done' })).toStrictEqual({ error: 'denied' });
	});
});
