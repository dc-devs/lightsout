...
describe('listEligibleTickets', () => {
	test('asks one query per route label, so the route is known from the query rather than a second round trip per ticket', async () => {
		...
		expect(tickets).toStrictEqual([
			{ id: 'id-70', identifier: 'LO-70', title: 'Ticket 70', description: '', priority: 2, createdAt: '2026-01-01T00:00:00.000Z', route: 'direct' },
			{ id: 'id-71', identifier: 'LO-71', title: 'Ticket 71', description: '', priority: 2, createdAt: '2026-01-01T00:00:00.000Z', route: 'auto-plan' },
		]);
	});
