...
import { listEligibleTickets as listJiraEligibleTickets } from '#src/queue/tracker/jira/index.ts';
import { listEligibleTickets as listLinearEligibleTickets } from '#src/queue/tracker/linear/listEligibleTickets.ts';
...
export const listEligibleTickets = async ({ settings }: Params): Promise<TicketSummary[] | QueueFailure> => {
	return settings.tracker === 'linear' ? listLinearEligibleTickets({ settings }) : listJiraEligibleTickets({ settings });
};
