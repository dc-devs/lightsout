...
export const serializeAttachmentManifest = ({ files, planningGeneration, brainstormGeneration }: Params): Buffer =>
	Buffer.from(
		`${JSON.stringify(
			{
				schemaVersion: 1,
				files: files.map(({ name, content }) => ({ name, sha256: sha256({ content }) })),
				...(planningGeneration === undefined ? {} : { planningGeneration }),
				...(brainstormGeneration === undefined ? {} : { brainstormGeneration }),
