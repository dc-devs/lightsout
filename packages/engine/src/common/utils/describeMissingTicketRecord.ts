 ...
 * here rather than in the ticket module because `lightsout ticket show` reads a
...
export const describeMissingTicketRecord = ({ ticketBranch }: Params): string =>
	`there is no ticket record for '${ticketBranch}' — start one with \`lightsout ticket add-plan --name ${ticketBranch} --slug <slug>\`, adding \`--from ${ticketBranch}\` when that folder already holds the loose files the plan is to be made out of`;
