...
import { setParkedLabel } from '#src/ticketTracker/jira/setParkedLabel.ts';
...
describe('Jira setParkedLabel', () => {
	test('does nothing when no parked label is configured', async () => {
		...
		expect(await setParkedLabel({ settings: jiraTrackerSettingsFixture(), ticketId: 'LO-1', label: undefined, parked: true })).toBeUndefined();
	...
	test.each([
		...
		expect(await setParkedLabel({ settings: jiraTrackerSettingsFixture(), ticketId: 'LO/1', label: 'queue-parked', parked })).toBeUndefined();
	...
	test('passes REST failures through unchanged', async () => {
		...
		expect(await setParkedLabel({ settings: jiraTrackerSettingsFixture(), ticketId: 'LO-1', label: 'queue-parked', parked: true })).toStrictEqual({
			error: 'denied',
		});
	});
});
