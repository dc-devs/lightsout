...
describe('PathVerification', () => {
	...
	test('the three miss lists default to empty — a clean check is still a verification', () => {
		...
		// fields
		expect(parsed).toStrictEqual({ pathsChecked: 4, missingPaths: [], scriptsChecked: 2, missingScripts: [] });
	});
	...
	test('zero checks parse — an authored facts file with no paths to verify is a valid result', () => {
		...
		expect(parsed).toStrictEqual({ pathsChecked: 0, missingPaths: [], scriptsChecked: 0, missingScripts: [] });
	});
	...
	for (const field of ['missingPaths', 'missingScripts']) {
