...
test('ConfigGates: a block carries every command through parsing, with test holding the fast red/green command', () => {
	...
	expect(ConfigGates.parse({ check: 'c', test: 't', testCoverage: false })).toStrictEqual({ check: 'c', test: 't', testCoverage: false });
});
...
test('ConfigGates: check, test, and testCoverage are each required', () => {
	...
	expect(ConfigGates.safeParse({ check: 'c', testCoverage: false }).success).toBe(false);
	expect(ConfigGates.safeParse({ test: 't', testCoverage: false }).success).toBe(false);
	...
	expect(ConfigGates.safeParse({ check: 'c', test: 't', testCoverage: true }).success).toBe(false);
});

test('ConfigGates: a stale testUnit key is refused with a message naming its new name', () => {
	const result = ConfigGates.safeParse({ check: 'c', test: 't', testUnit: 't', testCoverage: false });
