...
export const runDoctor = async ({ cwd, probeHarness }: Params) => {
	...
	// The standards' mechanical rules (import type, no any) assume the
	// consumer's linter enforces them — lightsout ships no lint preset (hard
	// rule), so the doctor is where the gap surfaces. A consumer that opted
	// out of standards entirely (standards: false) has opted out of this too.
