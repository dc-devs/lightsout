...
const findLowerPlanBlocker = ({ record, planId }: { record: WorkOrderState; planId: string }) => {
	...
	return blocking === undefined
		? undefined
		: `plan ${blocking.id} comes before ${planId} on work order ${record.branch} and its implementation has not finished, and a work order's plans implement in numeric order — finish it with ${describeLowerPlanRemedy({ branch: record.branch, plan: blocking })}`;
};
...
export const findPlanImplementationBlocker = ({ record, planId }: Params): string | undefined => {
	...
	if (plan === undefined) {
		blocker = `work order ${branch} holds no plan ${planId} — \`lightsout work-order show --name ${branch}\` lists the plans it does hold`;
	} else if (plan.exclusion !== undefined) {
		blocker = `plan ${planId} is excluded from work order ${branch} — ${plan.exclusion.reason} — and an exclusion is final; add follow-up work as a new plan with \`lightsout work-order add-plan --name ${branch}\``;
	} else if (record.mode === WorkOrderMode.SinglePlan && planNumberOf({ id: planId }) !== 1) {
		blocker = `work order ${branch} is in single-plan mode, where plan 001 alone supplies the implementation, so plan ${planId} is not built — run \`lightsout work-order mode --name ${branch} --set multiple-plan\` to implement this ticket's plans in numeric order`;
	} else if (plan.progress === PlanProgress.Implemented) {
		blocker = `plan ${planId} is already implemented on work order ${branch}, and an implemented plan is the scope the ticket ships on; add follow-up work as a new plan with \`lightsout work-order add-plan --name ${branch}\``;
	} else {
