...
import { findLiveRunRefusal } from '#src/workOrder/common/planSource/findLiveRunRefusal.ts';
import { readLooseFileProgress } from '#src/workOrder/common/planSource/readLooseFileProgress.ts';
import { readLooseFileRuns } from '#src/workOrder/common/planSource/readLooseFileRuns.ts';
...
export const resolvePlanSourceFolder = async ({ cwd, from }: Params): Promise<PlanSourceFolder | { error: string }> => {
	...
	if (entries.length === 0) {
		...
		// files from its ticket is `restoreWorkOrderPlan`'s job rather than this one's.
