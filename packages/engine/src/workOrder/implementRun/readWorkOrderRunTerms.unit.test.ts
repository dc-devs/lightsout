...
/** A work order folder whose `state.json` is not JSON at all, the shape a hand edit or a half-copied file leaves. */
...
describe('readWorkOrderRunTerms', () => {
	...
	test('holds back the ship chain for a run of one phase file of a ticket plan', async () => {
		...
		const onePhase = await readWorkOrderRunTerms({
			cwd,
			name: addressOf({ planId: '001-record' }),
			planPath: planFileOf({ planId: '001-record', file: 'phase2-plan-addressing.md' }),
		});
		const wholePlan = await readWorkOrderRunTerms({
			cwd,
			name: addressOf({ planId: '001-record' }),
	...
	test('answers a refusal and a ship blocker when the work order state cannot be read', async () => {
