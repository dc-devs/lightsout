...
/**
 * A rule's check, written the way a package author writes one: annotations and
...
A file named banned.ts is the example this package exists to refuse.
`;

/**
 * A one-rule package on disk. `passFiles` is what the check must accept — set it
 * to the banned name to build the package whose check cries wolf.
 */
const setupPackage = async ({ passFiles = ['allowed.ts'] }: { passFiles?: string[] } = {}) => {
	const packagePath = await mkdtemp(join(tmpdir(), 'lightsout-standards-package-'));
	const rulePath = 'code/demo/01-no-banned-file';
	const files: Record<string, string> = {
		'lightsout-standards.json': '{ "name": "demo-standards", "formatVersion": 1 }\n',
		// A standalone package declares its own module format, so its checks load
...
test('cli: standards-validate loads a package, runs its check against its fixtures, and exits 0', async () => {
	const { packagePath } = await setupPackage();

	const { stdout, stderr, code } = await runCli({ args: ['standards-validate', '--package', packagePath] });
...
test('cli: standards-validate names the rule whose check flags its own pass fixture and exits 1', async () => {
	const { packagePath } = await setupPackage({ passFiles: ['banned.ts'] });
...
test('cli: standards-validate reports a package it cannot load and exits 1', async () => {
	const packagePath = await mkdtemp(join(tmpdir(), 'lightsout-standards-empty-'));

	const { stdout, stderr, code } = await runCli({ args: ['standards-validate', '--package', packagePath] });

	expect(stdout).toBe('');
	expect(stderr).toMatch(/standards package root file not found/);
	expect(code).toBe(1);
});
