...
// fail. The plan lives in its own folder inside its work order's plans folder,
// derived from cwd and the address alone, and the shared cleanPlanBody's paths
// resolve against what this seeds.
const seedPlanLintFixture = async ({ body }: { body: string }) => {
	const cwd = await freshCwd();
	const planDir = join(cwd, '.lightsout', 'work-orders', 'demo', 'plans', '001-widget');

	await mkdir(join(cwd, 'src'), { recursive: true });
	await mkdir(planDir, { recursive: true });
	await writeFile(join(cwd, 'src', 'index.js'), 'export const one = 1;\n', 'utf8');
	await writeFile(join(planDir, 'plan.md'), body, 'utf8');
	writeEmptyDecisions({ dir: planDir, name: planAddress });
...
test('cli: plan lint on a clean plan reports clean and exits 0', async () => {
	...
	const { stdout, stderr, code } = await runCli({ args: ['plan', 'lint', '--name', planAddress, '--no-worktree', '--cwd', cwd] });

	expect(stderr).toBe('');
	expect(stdout).toMatch(/plan lint demo\/001-widget: 0 blocking, 0 advisory finding\(s\) across 1 file\(s\)/);
	expect(stdout).toMatch(/plan lint demo\/001-widget — clean \(1 file\(s\)\)/);
...
test('cli: plan lint on a plan with a placeholder prints the finding and exits 1', async () => {
	...
	expect(stderr).toBe('');
	expect(stdout).toMatch(/plan lint demo\/001-widget — 1 blocking finding\(s\) \(1 file\(s\)\)/);
...
test('cli: plan lint without a plan deliverable reports the error and exits 1', async () => {
	...
	const { stdout, stderr, code } = await runCli({ args: ['plan', 'lint', '--name', 'ghost/001-widget', '--no-worktree', '--cwd', cwd] });

	expect(stdout).toBe('');
	expect(stderr).toMatch(/no plan found for 'ghost\/001-widget'/);
	expect(code).toBe(1);
});
