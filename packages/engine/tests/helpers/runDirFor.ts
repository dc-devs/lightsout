 ...
 */
export const runDirFor = ({ cwd, runId, planName, ticketBranch, pipeline }: Params): string => {
	const ticket = planName === undefined ? ticketBranch : planName.split('/')[0];
	const runsDir = ticket === undefined ? join('.lightsout', commandFolders[pipeline ?? 'implement'], 'runs') : join('.lightsout', 'tickets', ticket, 'runs');
