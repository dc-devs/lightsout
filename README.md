...
| `traverse.connections` | no | Where the traverse connection map lives: a local dir, a git URL (`git@github.com:org/data-flow-map`), or a folder inside a repo (`git@github.com:org/platform/src/connections`). Central-first: git sources are cloned/refreshed automatically so everyone traverses the map's current HEAD. Default `.lightsout/connections` (local). See [docs/traverse.md](docs/traverse.md). |
...
| `lightsout traverse "<question>" --start <edge-or-node> [--connections <dir>] [--budget <n>] [--data <field>] [--cwd <path>]` | Cross-repo data-flow traversal: answers questions no single repo can by following data through a committed map of **connection docs** (one small file per process-boundary edge — HTTP, queues, S3, responses — with machine-checkable anchors). The engine owns the worklist loop (frontier + visited set + hop budget, trace state on disk after every hop — resumable via `--run <id>`); each hop is one bounded agent that traces one repo and is structurally unable to recurse. Unmapped exits are reported as GAPs, never guessed through; drifted anchors are named for repair. Map lives in `.lightsout/connections/` by default (`repos.yaml` + one doc per edge). **Guide: [docs/traverse.md](docs/traverse.md)** — setup, map building, the review gate, maintenance, and the doc format. |
| `lightsout build-map <node...\|all> [--connections <dir>] [--rescan] [--cwd <path>]` | Builds/extends the traverse map: one scan agent per node in parallel inventories every process-boundary edge (inventories saved durably; re-runs are SHA-gated and skip current nodes), then a mechanical join pairs sightings — every real edge is seen twice, outbound + inbound, so joined edges are born with both anchors code-verified. Ends at the **review gate**: `join.json` (new/confirmed/drifted/orphans/noise), no docs written. After you cull it: `lightsout build-map --author <run-id>` writes the connection docs, applies confirmed/drift repairs, and regenerates INDEX.md. Re-running the join doubles as the map's verification sweep. |
| `lightsout map-connection verify [<doc-id>...] [--repair]` | The map's freshness sweep as pure code — no agent, no clone unless a repo's HEAD actually moved (one `ls-remote` per repo gates everything). Per anchor: `current` (SHA unchanged), `ok` (pattern at the anchored path), `drifted` (found elsewhere — `--repair` repoints it), or `missing` (reported for a human; never auto-deleted). Cheap enough to cron. |
| `lightsout map-connection draft --run <traverse-run-id>` | Every traversal should improve the map: turns a trace's GAPs (unmapped exits with concrete citations) into draft connection docs under `drafts/` — the from-side is code-verified, the to-side is the human blank. Drafts never route traversals until moved up into the connections dir. |
| `lightsout plan explore "<request>" --name <n> [--areas <a,b>] [--cwd <path>]` | Planning phase, stage 1: explorer agents gather codebase facts for the feature (one per `--areas` entry), then the engine **deterministically verifies** every returned path/script on disk before persisting `.lightsout/plans/<n>/facts.json`. Usually driven by the `/plan` skill, not run by hand. |
