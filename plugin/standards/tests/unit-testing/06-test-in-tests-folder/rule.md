...
- **A test imports its subject from the subject's own file**: a test beside its
  source imports the file it tests (`./AuthService`), never the module's barrel
  (`./index`) — an import through a barrel loads every file the module
  re-exports to reach one name. A test inside a module may import that
  module's other files too; a test outside it imports only the files its
  barrel exports. A barrel entry whose only consumers are test files is
  legitimate public API — it marks a deliberate promotion whose contract the
  tests pin; demoting it is a human decision, not a dead-code cleanup.
