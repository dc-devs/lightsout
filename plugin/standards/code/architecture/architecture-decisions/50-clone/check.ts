import type { CloneSpan, RawStandardsFinding, StandardsCheckModule } from '@/contracts';
...
export const check: StandardsCheckModule = {
	...
	// the paths, so a second copy-paste between two files already flagged is more
	...
	run: ({ input }): RawStandardsFinding[] => {
		...
		return [...groupByPair({ spans }).values()].map(({ files, spans: count, longest }) =>
			buildRawFinding({
				rule: 'clone',
				files,
				detail: `${count} duplicated span(s), the longest ${longest} lines`,
