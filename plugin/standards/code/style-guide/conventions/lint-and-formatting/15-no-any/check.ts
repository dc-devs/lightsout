...
/** One finding per file: the work is "open this file and give its values real types", which does not become three jobs because three annotations say `any`. */
const buildFileFindings = ({ input }: { input: SyntaxTreeInput }) => {
	const findings: RawStandardsFinding[] = [];

	for (const [path, tree] of input.trees) {
		const lines = findAnyLines({ sourceFile: tree, compiler: input.compiler });
		...
		if (lines.length > 0) {
			findings.push(
				buildRawFinding({
					rule: 'no-any',
					files: [{ path }],
					detail: `\`any\` at ${lines.length > 1 ? 'lines' : 'line'} ${lines.join(', ')}`,
