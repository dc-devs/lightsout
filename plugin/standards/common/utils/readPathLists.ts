 ...
 * arise.
 */
export const readPathLists = ({ input }: Params): { files: string[]; tests: string[] } =>
	input.kind === 'file-list' || input.kind === 'file-text' ? { files: input.files, tests: input.tests } : { files: [], tests: [] };
